...
describe('UI Baseline App', function() {
    ...
    if ( result ) {
      ...
      browser.waitForAngular();
       ...
       browser.sleep(10000);
            ...
            // ach card  editing
                  ...
                  var editing = element(by.xpath("//ecb-payment-methods/div/div[2]/div[1]/div[3]/div[2]/ul/li[1]/button[@disabled='disabled']"));
                   editing.isPresent().then(function(result) {
                 if ( result ) { 
                 console.log('It is master card');
                  console.log('edit is disabled,maybe payment is scheduled for this card');
            ...
            element(by.xpath("//div[@ng-if='vm.paymentMethods'][1]/div[3]/div[2]/ul/li[1]/button/span[contains(text(),'Edit')]")).click();
     ...
     browser.executeScript('arguments[0].scrollIntoView()',widget.getWebElement());
      ...
      expect(browser.getTitle()).toEqual(testdata.url.DashBoard);
      // edited or not validation
       expect(element(by.xpath("//div[@ng-if='vm.paymentMethods'][1]/div[3]/div/div[4]/span[2]")).getText()).toEqual(testdata.changedaccounttype);
               }
         else {
              console.log('there is some error in the request contact customer care');
              browser.actions().mouseMove(element(by.xpath("//p[contains(text(),'There was an error launching page to process your request.')]"))).perform();
